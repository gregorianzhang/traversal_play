...
class GoogleplaySpider(Spider):
    ...
    start_urls = (
        'https://play.google.com/store/apps/category/GAME_STRATEGY/collection/topselling_free?start=0&num=24',
        )
    global category1
    global url1
    category1 = 'GAME_STRATEGY'
    url1 = 'https://play.google.com'

    def parse(self, response):
        if re.match(".*play\.google\.com.*",response.url):
            for pagenum in range(0,2000,24):
